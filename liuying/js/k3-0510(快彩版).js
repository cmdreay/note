...
{
	var myDate = new Date()
	var curSecondOfDay = parseInt(myDate.getHours())*60*60+parseInt(myDate.getMinutes())*60+parseInt(myDate.getSeconds())	//当前时间秒数
...
function adjustTitleDivSize()
{
	...
	if (curLotteryTypeStyleName.indexOf("快彩")>=0) {	
		...
		kuaicaiDiv.style.width = "578px"
		kuaicaiDiv.style.marginLeft = "145px"
		...
		kuaicaiDiv.style.fontSize=(captionHeight/2-3)+"px"
		kuaicaiDiv.innerHTML = curLotteryTypeName+"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"+"左右键切换页面"+"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"+"客服热线 : "+agentphoneNumber
	}else{
	...
	if (curLotteryTypeName.indexOf("天津快乐十")>=0) {		
		...
		imgObj.src="images/logo88.png"				
		imgObj.style.marginLeft="14px";
...
function createPrizeNumDistribution(length,ballflag,typek3,typek12Qian,typej7l1,typej7l2,typek12,typek12JD,type11x5Green,typek3ShuZi)
{
	...
	}else if (curLotteryTypeStyleName.indexOf("快彩")>=0) {// 0509快彩开奖号码分布 背景色
		cell.style.backgroundColor="white"
	}
	else{
				...
				if(titleTxt === "和值走势图"&&maxSum === 18){
					if (i<11) {
						if(curLotteryTypeStyleName.indexOf("艺彩")>=0){
							...
							td.style.backgroundColor = "rgb(237, 241, 251)"	
						}
...
function initWebSocket()
{
	...
	ws = new WebSocket("ws://123.57.212.33/wsapp/push?uid="+localStorage.sn)//创建Socket实例
