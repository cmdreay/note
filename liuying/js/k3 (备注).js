...
var gVersion= "3.426" //版本号
var holdSnChar="******"

var updateTableContentFlag=false////更新表内容标志flag 后续没有用到  可忽略
var updateTableContentInterval=0//更新表内容间隔  后续没有用到  可忽略

var updateWebsocketStateFlag = true//更新websocket状态flag 帐号异常（下线、到期等）时置为false 当为false时 不需要重新连接了，进入模态了，需要客服重启处理了

var systemUpdateFlag=false//systemUpdateFlag  //系统是否需要更新 更新号码时置为true 当为true时去更新号码
			
var backgroundNumberEnabled=false//没有用到  可忽略
...
//createInnerTable() 创建每一个元素表格  每一个创建元素的函数都会用到
...
//////////////////////////////////
var accountExpiringtipsVisible=false//账户即将到期提示  党委true  即帐号到期  floatDiv就显示为block将信息遮挡住
var accountExpiringtipsInterval=10//如果账户即将到期  有10秒倒计时
var expirydate//显示出来还有多久到期

//帐号异常提示信息函数  帐号出现异常会显示执行函数  
...
//右上角倒计时计算函数 所有高频彩都用这个
function getK11x5LeftTime()
{
	var myDate = new Date()
	var curSecondOfDay = parseInt(myDate.getHours())*60*60+parseInt(myDate.getMinutes())*60+parseInt(myDate.getSeconds())	
...
var imgObj//logo图片obj
var titleDiv//表头文字div
var styleTitleDiv//版式信息文字
// var countDownDiv

var progressDiv//倒计时变灰的百分比变量
var timeDiv//倒计时div
//更新表头彩种文字信息函数  例如“北京11选5“
...
var kuaicaiDiv//0510 快彩 表头部分  和普通的有所区别  设置尺寸显示内容等
function adjustTitleDivSize()
{
	...
	// if (curLotteryTypeName.indexOf("天津11选5")>=0) {//0510 其他地方设置了  这个位置不需要重复设置
 ...
 */
{
	...
	{
		progressContainer = document.getElementById("progressContainer")//0510 倒计时部分backgroundcolor
	}

	if (curLotteryTypeName.indexOf("天津11选5")>=0) {
		progressContainer.style.backgroundColor = "#ff03fb"
	}else{
		progressContainer.style.backgroundColor = "blue"
	}
	...
	timeDiv.style.width = progressContainer.offsetWidth+'px';
	progressDiv.style.width=progress+"%"
	//console.log("updateProgressInfo:progress ",progressDiv.style.width)
	timeDiv.innerHTML = timeStr

	var issueString = curLotteryTypeDataList[curLotteryTypeDataList.length-1].issue
	var issue = parseInt(issueString.substring(issueString.length-2))+1
	
	var hour = timeStr.substring(0,2)
	var second = timeStr.substring(3,5)

	kuaicaiTimeDiv.innerHTML ="下期销售剩:"+"<span style='color:red'>"+hour+"</span>"+"分"+"<span style='color:red'>"+second+"</span>"+"秒"//timeStr
	// kuaicaiTimeDiv.innerHTML ="下一期销售剩："+timeStr


}

//大乐透开奖时间  大乐透开奖公告[每周一、三、六20:30开奖] 20:00销售截止
...
//七星彩开奖倒计时  [每周二、五、日20:30开奖] 20:00销售截止
...
function getPl35Or3DLeftTime()//排三排五开奖时间
{
	//每天晚上20：00销售截止
...
var notifyList=["this is the first notify","this is the second notify"]//存放通知的内容
...
//////////////////////////////////
var tipsInfoDiv  //免责声明的div
//agentphone
var agentphoneNumber//报装电话
function showTipsInfo()//免责声明函数  包括显示版本号 免责声明  包装电话
{
...
//////////////////////////////////
var adList//存放所有广告的list 
var curAdIndex=0//多个广告会轮播 记录当前是地几个广告
var adSwitchInterval = 3000 //广告轮播时间
var curAdSwitchInterval=0//

var adImgObj//广告图片
var adHDivContainer//就是放遗漏或广告的div
//更新广告的函数
...
var divObjyl = document.createElement("div")//之前divObj是一个全局变量,复用率较高,现在轮播需要分开单独存放.
function createYiLouArea(){//添加遗漏区域函数  设置尺寸
...
var advertisingArea = false//判断是否有广告的flag  为广告遗漏轮播服务
var divObjad = document.createElement("div")
function createAdvertisingArea()//添加广告区域函数
{
	...
	return divObjad//返回一个空的广告或遗漏区域  方便后续添加内容进去
	
}
var divObjk10 =document.createElement("div")  	
function drawTianJinK10Foot(){//天津快十特殊底部
...
function drawK3StaticTable(){	//画快三表格函数
	//liutest begin
...
var fakek3YCYLDate = {one:{},two:{},three:{},sum:{}}//定义k3遗漏数据格式
function drawYCK3StaticTable() {////画艺彩快三表格函数  和11选5相像
	//liutest begin
...
function drawStaticTable(){	//简单 11选5 的遗漏表格
	//liutest begin1
...
function addAdvertisingArea(tableObj)//添加广告或遗漏区域  根据config来具体画遗漏或广告
{
	var tmpName = curLotteryTypeName
...
//advertising area end
//////////////////////////////////
function getCellHTMLk3(type,w,h,v){//设置表格字体大小函数  这个函数是用于只需要元素标题  不要数字显示的函数
	/**
	 * type classname  执行css
	 * w 宽
	 * h 高
	 * v 内容
	 */
...
//设置表格字体大小函数  后续表格元素内只要添加数字就需要使用这个函数  
...
/**
	 ...
	 * h 高
	 * v 内容
	 * 其他的均为falg 都是布尔 不需要特殊样式可不添   传入false或者true
	 * 不同的flag分别对应执行不同的代码 用来实现不同的样式 详情表现出怎样的样式或执行哪一块代码见下面代码可知 
	 * 
	 */
{
	var minVal = Math.min(w,h)
...
/**
 * (tableObj,titleTxt,w,h,minVal,maxVal,typeone,typej7,typek10,typek10Green,typek10Green3,typek3ZC,typek3GJ,typek3JB,typek3JB1,typej7l1,typej7l1JHZS,typej7l2,typek12JD,typek12JDHistory,typeJLK3Danshuang,typeJLK3012,type11x5GreenZuXuan,type11x5Green2,type11x5KDZS,big30Small)
 * tableObj  mtable  就是一个table
 * titleTxt 表头文字标题
 * w 宽
 * h 高
 * minVal 开奖号最小值
...
////添加分析连号矩阵数据 开始
//////////////////////////////
var needInitNumberMatrix = true//needInitNumberMatrix 是否分析连号矩阵数据
var numberMatrix=[];//存放连号矩阵数据

function adjustColIndex(colIndex)//计算连号的函数
{
	if(prizeNumMin==0)
	{
		return colIndex;
...
var lotteryStyleList//彩票类型list
function resetTable()//每次init初始化时  就把原来画好的元素删除等待重新加载新的元素
{
	//liutest begin
...
function initTableContent()//初始化函数
{
	...
	for(var i in componentList)//遍历componentlist  所有元素对应的数字  执行对应的函数  将元素显示出来
	{
		switch(componentList[i])
		{
			case "1":
	...
	//判断当前的彩种是否是有遗漏的彩种   高频彩一般都有遗漏   低频彩没有遗漏
	...
	showTipsInfo()//判断帐号是否异常 显示信息
	
	if(notificationAreaEnable)  //in tfootArea  有通知就添加通知 将通知显示出来
		addNotifyArea(rootTable)
	if(advertisingAreaEnable)  {//in tfootArea  有遗漏或广告就执行函数 将遗漏或广告显示出来
		addAdvertisingArea(rootTable)
	}
	coverArea()//试用版的覆盖函数   如果试用期到了就会显示
	justUpdatePrizeNumAreaFlag=false
}
//520 520升级遮挡 start
var sjzhedang
function shengjiZhedang(){//520升级遮挡  可忽略
...
//不同键值对应一定的操作 上下左右等操作
function handleKeyUp(e)//e就是键值
{
	var lotteryName1,lotteryName2; 
	var keynum
	var keychar
...
function requestLotteryData(name,count)//数据的请求响应
{
	...
	overdueDiv = document.getElementById("overdueDiv")//试用版覆盖位置
...
function yilouDataRequest(){//20170414 遗漏数据加载 function  请求响应
	var url = ''
	var name = curLotteryTypeName
	if(name.indexOf('11选5')>=0 || name.indexOf('快乐彩')>=0){
...
function _handleSysEvent(messageObj)
{
	...
	switch(messageObj.code) 
	{
		...
		case 3: // 被踢下线
			...
			updateWebsocketStateFlag=false //不需要重新连接了，进入模态了，需要客服重启处理了
			systemUpdateFlag = false //系统不需要再更新了
			showSysEventFloat(messageObj.code,"<span style='font-size:44px;'>您已下线！</span> <img  src='images/warning.png' style='width:50px;height:50px;' /> <br/><span style='font-size:20px;'>原因一：当前账户已在其他地方登录，您被迫下线！<br>原因二：您长期没有关机，系统将您强制下线！<br>您只需重启一下机顶盒即可继续使用<br />如果账户存在安全问题请联系"+agentphoneNumber+"</span>")
			break;
	}
	/*updateWebsocketStateFlag=false //不需要重新连接了，进入模态了，需要客服重启处理了
	systemUpdateFlag = false //系统不需要再更新了
...
function handlePushMessage(messageObj)//处理推送消息
{
	...
	switch(messageObj.cmd)
	{
		...
		case "configchanged":
			...
			break;
		case "sysevent":
			_handleSysEvent(messageObj)//帐号异常
			break;
		case "publish":
...
function updateLeftTime()//更新处理各中彩票计时
{
...
////////////////////
//show debug info
function showDebugInfo()//debug函数 可忽略
{
...
function doCheckRequest()
{
	...
	//checkXmlHttp = new XMLHttpRequest()
	//checkXmlHttp.onreadystatechange = handleDoCheckRequest
	checkXmlHttp.open("HEAD","http://101.200.168.40/sporttery/",true);
	checkXmlHttp.send();
}
...
////// start//////元素函数  从3549行开始为元素函数    到行元素函数截止

function createPrizeIssue(typek10Green,typek10Aver2,typek3ZC,typek12JD,typek12JDHistory,typek3ZCHistory,type11x5Green)
{
/**
 * 都是布尔型
 * (typek10Green,typek10Aver2,typek3ZC,typek12JD,typek12JDHistory,typek3ZCHistory,type11x5Green)
 * @param {*} typek10Green k10绿色版
 * @param {*} typek10Aver2 k10两区版
 * @param {*} typek3ZC k3珍藏版
 * @param {*} typek12JD  k12经典版
 * @param {*} typek12JDHistory k12经典版历史
 * @param {*} typek3ZCHistory k12珍藏版历史
 * @param {*} type11x5Green 11选5绿色版
 * 不同的flag分别对应执行不同的代码 用来实现不同的样式 详情表现出怎样的样式见代码可知
 */	
...
{
	/**
	 * length,preAreaLength,testPrizeFlag,typek3,typek10Aver4,typek10Green,typek10Aver2,typek3ZC,typek12JD,typek12JDHistory,typeOddBig,typePaiLie3,historyOddBig
	 * length：传入的为int 表示一共多少个开奖号码
	 * preAreaLength  传入的为int 表示前区一共多少个开奖号码 或者要显示多少个号码
	 * 其他的均为falg 都是布尔   传入false或者true
	 * 不同的flag分别对应执行不同的代码 用来实现不同的样式 详情表现出怎样的样式或执行哪一块代码见下面代码可知 
	 * 
	 */
	var tmpWdithRedundantPixels
...
{
	/**
	 * 开奖号码走势  也就是奖号分布 不考虑开奖顺序将其按从小到达顺序显示出来
	 * length,ballflag,typek3,typek12Qian,typej7l1,typej7l2,typek12,typek12JD,type11x5Green,typek3ShuZi)
	 * length 传入int 表示一共显示多少个数字
	 * ballflag  boole布尔 判断是否以球形显示
	 * 其他的均为falg 都是布尔   传入false或者true
	 * 不同的flag分别对应执行不同的代码 用来实现不同的样式 详情表现出怎样的样式或执行哪一块代码见下面代码可知 
	 * 
	 */
...
/**
 * 计算重号的函数
 * prevPrizeNum 上一组开奖号码
 * curPrizeNum 当前这组开奖号码
 * length 所计算的号码数
 * 返回值repeatNums 即为两组号码重号个数
 */
{
...
function createPrizeNumRepeatTimes(length,typek10Aver4,typek10Green,typek10Aver2,typek12Qian,typek12JD,type11x5Green)
{
	/**
	 * 重号
	 *length,typek10Aver4,typek10Green,typek10Aver2,typek12Qian,typek12JD,type11x5Green)
	 *length 开奖号码的个数
	 * 其他的均为falg 都是布尔   传入false或者true
	 * 不同的flag分别对应执行不同的代码 用来实现不同的样式 详情表现出怎样的样式或执行哪一块代码见下面代码可知 
	 * 
...
function createPrizeNumSum(length,typek12Qian,typek12JD,type11x5Green)
{
	/**
	 * 开奖号码和值
	 * length,typek12Qian,typek12JD,type11x5Green)
	 * length 传入int 表示一共显示多少个数字
	 * 其他的均为falg 都是布尔   传入false或者true
	 * 不同的flag分别对应执行不同的代码 用来实现不同的样式 详情表现出怎样的样式或执行哪一块代码见下面代码可知 
	 * 
...
function createOneOfPrizeNum012Trend(position,bgColor)
	/**
	 * 单独一个号码的012走势函数
	 * position  int  必填  用于显示第几个号码的
	 * bgColor  传入颜色 eg：”red""  背景颜色
	 */
{	
	var ballsCount=new Array()  //for 统计
	for(var i=0;i<3;i++)
...
function getPrizeNumOdd(prizeNum,length)//计算奇数函数
/**
	 * 计算奇数函数 prizeNum,length 必填项
	 * prizeNum 开奖号码
	 * length int 必填 开奖号码的长度
	 * 返回值m为计算出的奇数个数
	 */
...
function createBackAreaPrizeNumDistribution(typeDLT2)//20161009大乐透后区 同前区一样  
{
	var ballsCount=new Array()  //for 统计
...
function createBackAreaPrizeTrend()//低频彩后区号码走势
{
	var div = document.createElement("div")
	div.style.position='relative'
...
function createAver4PrizeNumDistribution()//四分区号码分布
{
	var div = document.createElement("div")
	div.style.position='relative'
...
function createAver2PrizeNumDistribution()//两区版号码分布
{
	var ballsCount=new Array()  //for 统计
	for(var i=0;i<prizeNumMax;i++)
		ballsCount[i]=0
...
function createj7PrizeNum(length,typej7l2)//金七乐号码显示
{	
	var tmpWdithRedundantPixels
	if(trendCellCount==0 && trendWidthRedundantPixels>0)
	{
...
function createMahjongDistribution(){//黑龙江麻将分布函数
	var ballsCount=new Array()  //for 统计
	var lastDataCount = new Array()
	var curDataCount = new Array()
	for(var i=0;i<prizeNumAvailableNumCount;i++)
...
function createk10GreenPrizeNumDistribution(length)//快乐十分绿色版开奖号码分布  length必填项  开奖号码几位
{	
...
function getPrizeNumBigTimes(prevPrizeNum)//0509快彩大号计算函数
{

	var repeatNums = 0;
	var repeatArr=[] ;
	for (var i = 0; i < prevPrizeNum.length; i++) {
		if (parseInt(prevPrizeNum[i])>5) {
			repeatNums++	
		}
	}
	return repeatNums
